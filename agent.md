...
- 单场胜平负概率当前会加权赔率/市场数据：如果存在 `odds_snapshots.market='1x2'`，先用 `no_vig_probabilities()` 去水，再按模型侧 58% + 市场隐含概率 42% 融合；没有赔率数据时为纯模型口径。
- 前端 `/worldcup` 的分析页顺序是：核心判断、球员分析、胜负逻辑、比分预测/进球数倾向、其他风险与对位。
- 胜负逻辑不能只写“球队基础评分高”。需要体现模型计算路径：基础评分差、近期状态差、攻防匹配差、赛地/主场修正、市场隐含概率如何共同推到最终胜平负概率。
- `backend/main.py` 的 `model_logic_note()` 会生成稳定的模型口径说明；`normalize_report_content()` 会在 DeepSeek 输出过于泛化时补足模型计算说明。
- `app.js` 会把 `report.factors` 和 `report.oddsImplied` 渲染成模型因子卡片，辅助解释胜负逻辑。
