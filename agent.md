...
- Git 提交信息默认使用中文，除非用户明确要求英文或项目已有强制英文提交规范。
- 提交前先确认暂存范围，避免把日志、临时脚本、数据库、`.env` 或本地运行产物混入提交。
...
- 前台问号提示使用 `body` 下的 `.floating-tooltip`，由 `app.js` 按视口边缘夹紧定位；不要再用 `::after` 相对按钮居中显示，否则手机端靠边提示会超出屏幕。
...
- 比分 fact check 只匹配独立比分，不能把阵型 `4-2-3-1` 中的 `2-3` 误判为比分；正则需避免匹配前后仍连接数字或连字符的片段。
